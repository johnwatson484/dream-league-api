...
import { getHtmlStringFromFile } from '../../../notifications/get-html-string.ts'

const mockData = {
  gameweekId: 5,
  scores: [
    { managerId: 1, manager: 'John Watson', goals: 6, conceded: 1, margin: 5, result: 'W', scorers: [{ playerId: 1, name: 'Salah M', goals: 2 }, { playerId: 2, name: 'Palmer C', goals: 2 }, { playerId: 3, name: 'Saka B', goals: 1 }, { playerId: 4, name: 'Watkins O', goals: 1 }] },
    { managerId: 2, manager: 'Lee Gordon', goals: 3, conceded: 3, margin: 0, result: 'D', scorers: [{ playerId: 5, name: 'Haaland E', goals: 2 }, { playerId: 6, name: 'Isak A', goals: 1 }] },
    { managerId: 3, manager: 'Scott Dormand', goals: 1, conceded: 4, margin: -3, result: 'L', scorers: [{ playerId: 7, name: 'Solanke D', goals: 1 }] },
    { managerId: 4, manager: 'Mike Brearley', goals: 4, conceded: 2, margin: 2, result: 'W', scorers: [{ playerId: 8, name: 'Son H', goals: 2 }, { playerId: 9, name: 'Gordon A', goals: 1 }, { playerId: 10, name: 'Mbeumo B', goals: 1 }] },
    { managerId: 5, manager: 'Chris Sherwood', goals: 2, conceded: 5, margin: -3, result: 'L', scorers: [{ playerId: 11, name: 'Cunha M', goals: 1 }, { playerId: 12, name: 'Nkunku C', goals: 1 }] },
    { managerId: 6, manager: 'Dave Hartley', goals: 10, conceded: 1, margin: 9, result: 'W', scorers: [{ playerId: 13, name: 'Fernandes B', goals: 3 }, { playerId: 14, name: 'Diaz L', goals: 3 }, { playerId: 15, name: 'Johnson B', goals: 2 }, { playerId: 16, name: 'Wissa Y', goals: 2 }] },
  ],
  winners: [{ managerId: 6, manager: 'Dave Hartley', goals: 10 }],
  table: [
...
const routes: ServerRoute[] = config.get('isDev')
  ? [{
      method: 'GET',
